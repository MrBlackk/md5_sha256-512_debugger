...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/12/01 17:35:48 by vchornyi          #+#    #+#             */
/*   Updated: 2018/12/01 17:35:50 by vchornyi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	sum64(size_t num, int x, int y, int z)
{
	return (right_rotate64(num, x)
				^ right_rotate64(num, y)
					^ right_rotate64(num, z));
}

static size_t	ch64(size_t x, size_t y, size_t z)
{
	return (x & y) ^ (~x & z);
}

static size_t	maj64(size_t x, size_t y, size_t z)
{
	return (x & y) ^ (x & z) ^ (y & z);
}
...
void			permutation512(size_t *mem, t_buf64 *s)
{
	...
	while (i < 80)
	{
		t1 = s->bf[7] + sum64(s->bf[4], 14, 18, 41) + ch64(s->bf[4], s->bf[5],
					s->bf[6]) + g_sha512_const[i] + schedule[i];
		t2 = sum64(s->bf[0], 28, 34, 39) + maj64(s->bf[0], s->bf[1], s->bf[2]);
		s->bf[7] = s->bf[6];
		s->bf[6] = s->bf[5];
		s->bf[5] = s->bf[4];
		s->bf[4] = s->bf[3] + t1;
